...
def terminal_value(last_owner_earnings: Decimal | None, discount_rate: Decimal, terminal_growth_rate: Decimal, projection_years: int) -> TerminalValueResult:
    growth = available("terminal_growth_rate", terminal_growth_rate, "decimal_ratio", ("assumption:terminal_growth_rate",))
...
def fair_value(
    *,
    ...
    safety = available("margin_of_safety", margin_of_safety, "decimal_ratio", ("manual:margin_of_safety",))
    ...
    if equity.status != AVAILABLE or equity.value is None or shares.status != AVAILABLE or shares.value is None:
        ...
        safe = available("fair_value_after_safety_margin", fair.value * (Decimal("1") - margin_of_safety), "currency_per_share", fair.input_refs + safety.input_refs)
